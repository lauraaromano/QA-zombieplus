...
test('deve poder cadastrar um novo tvshow', async ({ page }) => {

  const tvshow = data.create
  await executeSQL(`DELETE FROM public.tvshows WHERE title = '${tvshow.title}';`)

  await page.login.do('admin@zombieplus.com', 'pwd123', 'Admin')
  await page.tvshows.goTv()
  await page.tvshows.create(tvshow)
  await page.popup.haveText(`A série '${tvshow.title}' foi adicionada ao catálogo.`)

})

test('deve poder remover um tvshow', async ({ page, request }) => {
